    ...
    <>
      <div className="pt-10">
         ...
         {useFullLinkData.slice(0,1).map((data)=>(
          <div className="py-4" key={data.id}>
             ...
             {data.post.map((posts,index)=>(
              <div key={index} className="pt-2 flex items-center justify-center">
         ...
         {useFullLinkData.slice(1,3).map((data)=>(
          <div className="py-4" key={data.id}>
             <Link to={data.link} target="blank" className="w-full h-[66px] text-headingText hover:text-blueActive flex items-center gap-4 border p-4 footer_bar duration-200">
         ...
         {useFullLinkData.slice(3).map((data)=>(
          <div className="py-4" key={data.id}>
             <Link to={data.link} target="blank" className="w-full h-[80px] flex items-center gap-4 border p-4 text-headingText hover:text-blueActive footer_bar duration-200">
             <img src={data.img} alt="" className="w-8 h-8 object-fill" />
                <div>
                <p className="font-medium">{data.description}</p>
